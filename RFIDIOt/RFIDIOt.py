	...
	def info(self,caller):
		...
		if not NoInit:
			...
			if self.readertype == self.READER_LIBNFC:			
				print 'LibNFC Reader' #+ self.n.libnfc.acName
			print
	#
	# reader functions
	#
...
#			time.sleep(.1)
	...
	def send_apdu(self,option,pcb,cid,nad,cla,ins,p1,p2,lc,data,le):
		...
		if self.readertype == self.READER_LIBNFC:
			result = self.n.sendAPDU(cla+self.ISOAPDU[ins]+p1+p2+lc+data+le)
